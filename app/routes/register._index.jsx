...
if (actionData?.message && typeof actionData.message === 'string') {
  ...
  if (actionData.message.toLowerCase().includes('confirmpassword must be longer')) {
    backendPasswordErrors.confirmPassword = 'Confirm password must be at least 6 characters';
  }
}
...
const validate = () => {
  ...
  if (!formData.fianceFirstName) newErrors.fianceFirstName = 'Fiance first name is required';
  if (!formData.fianceLastName) newErrors.fianceLastName = 'Fiance last name is required';
  ...
  if (actionData?.statusCode === 422 && actionData?.message?.toLowerCase().includes('email already exists')) {
  ...
  <StepsAndImage title="let's get to know each other." stepNo="1" totalSteps="9" content={ <div className="flex h-full items-center">
    ...
    <form className="space-y-6 max-w-full w-full mx-auto" onSubmit={handleSignup}>
      ...
      <div className="grid grid-cols-2 gap-4 max-[580px]:grid-cols-1">
        <Input value={formData.firstName} onChange={handleInputChange} label="First *" name="firstName" className="rounded-none p-5 border-[#B9B4AE] border-2 bg-white text-black w-full" classNameLabel="max-[580px]:text-left" error={errors.firstName} />
        <Input value={formData.lastName} onChange={handleInputChange} label="Last *" name="lastName" className="rounded-none p-5 border-[#B9B4AE] border-2 bg-white text-black w-full" classNameLabel="max-[580px]:text-left" error={errors.lastName} />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 max-[580px]:grid-cols-1">
        <Input value={formData.fianceFirstName} onChange={handleInputChange} label="First *"
          name="fianceFirstName" className="rounded-none p-5 border-[#B9B4AE] border-2 bg-white text-black w-full"  classNameLabel="max-[580px]:text-left" error={errors.fianceFirstName} />
        <Input value={formData.fianceLastName} onChange={handleInputChange} label="Last *"
          name="fianceLastName" className="rounded-none p-5 border-[#B9B4AE] border-2 bg-white text-black w-full" classNameLabel="max-[580px]:text-left" error={errors.fianceLastName} />
      </div>
      <div className="grid grid-cols-2 gap-4 max-[580px]:grid-cols-1">
        <Input 
          ...
          error={
            errors.email ||
            (actionData?.statusCode === 422 && actionData?.message?.toLowerCase().includes('email already exist') 
        ...
        <Input value={formData.confirmEmail} onChange={handleInputChange} placeholder="Confirm Email *" name="confirmEmail" className="rounded-none p-5 border-[#B9B4AE] border-2 bg-white text-black w-full"
          type="email" error={errors.confirmEmail} />
      </div>
      {/* <div className="text-center">
      ...
      <div className="grid grid-cols-2 gap-4 max-[580px]:grid-cols-1">
        <Input value={formData.password} onChange={handleInputChange} placeholder="Password *" name="password" type="password"
          className="rounded-none p-5 border-[#B9B4AE] border-2 bg-white text-black w-full" error={errors.password || backendPasswordErrors.password} />
        <Input value={formData.confirmPassword} onChange={handleInputChange} placeholder="Confirm Password *"
          name="confirmPassword" type="password" className="rounded-none p-5 border-[#B9B4AE] border-2 bg-white text-black w-full" error={errors.confirmPassword || backendPasswordErrors.confirmPassword} />
      </div>
