    ...
    <div className="max-w-2xl mx-auto p-6 bg-gray-100 rounded-lg shadow-md">
      ...
      {gifts.map((gift, index) => (
        ...
        >
          ...
          <p>
            Total Gift Value:{' '}
            <span className="font-medium">{gift.totalValue}</span>
          </p>
          <p>
            Contribution:{' '}
            <span className="font-medium">{gift.contribution}</span>
          </p>
          {gift.quantity !== undefined && (
            <div className="mt-2 flex items-center">
              <label htmlFor={`qty-${index}`} className="mr-2 font-medium">
                QTY
              </label>
              <input
                type="number"
      ...
      <h3 className="text-lg font-bold text-right mt-4">
        TOTAL:{' '}
        <span className="font-medium">${totalContribution.toFixed(2)}</span>
      </h3>

      <div className="mt-6 text-center">
        <ButtonComponent
          text="Confirm Gifts"
          onClick={() => alert('Gifts confirmed!')}
          className="w-full"
        />
      </div>
    </div>
